...
ALTER TABLE public.empleados DROP COLUMN IF EXISTS email;
...
ALTER TABLE public.movimientos
  ALTER COLUMN nro_remito TYPE text;
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_fecha_recambio_epp(p_desde date DEFAULT CURRENT_DATE)
RETURNS date
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
SET search_path = public
AS $$
  SELECT (p_desde + INTERVAL '6 months')::date
$$;

REVOKE ALL ON FUNCTION public.fn_fecha_recambio_epp(date) FROM PUBLIC;
...
BEGIN
  ...
  ELSIF v_tipo = 'Entrega_EPP' THEN
    ...
    NEW.es_devolucion := false;
    NEW.id_proveedor := NULL;
    NEW.fecha_recambio := COALESCE(NEW.fecha_recambio, public.fn_fecha_recambio_epp(CURRENT_DATE));
  END IF;
...
BEGIN
  ...
  BEGIN
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'rpc_iniciar_registro notif: %', SQLERRM;
  END;
...
GRANT EXECUTE ON FUNCTION public.rpc_depositos_activos_registro() TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_iniciar_registro(text, text, text, text, text, uuid[]) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_vincular_auth_registro(uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_crear_auth_registro(uuid, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_reenviar_codigo_registro(uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_cambiar_email_registro(uuid, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_confirmar_codigo_registro(uuid, text) TO anon, authenticated;
...
BEGIN
  ...
  IF v_mov.fecha_recambio IS NULL THEN
    v_mov.fecha_recambio := public.fn_fecha_recambio_epp(v_mov.fecha::date);
    UPDATE public.movimientos
...
BEGIN
  ...
  INSERT INTO public.movimientos (
    ...
    v_entrega,
    v_empleado,
    CASE WHEN v_tipo = 'Entrega_EPP' THEN public.fn_fecha_recambio_epp(CURRENT_DATE) ELSE NULL END
  )
...
BEGIN
  ...
  BEGIN
    ...
    END LOOP;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'rpc_actualizar_responsable notif: %', SQLERRM;
  END;
