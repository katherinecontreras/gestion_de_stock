...
export function explainRegistroError(errorOrMessage) {
    ...
    if (/PGRST202|Could not find the function|rpc_depositos_activos_registro|rpc_iniciar_registro|rpc_vincular_auth_registro|rpc_crear_auth_registro|rpc_reenviar_codigo_registro|rpc_cambiar_email_registro|rpc_confirmar_codigo_registro|42883/i.test(message)) {
...
export async function crearAuthRegistro(token, password) {
    const supabase = createBrowserClient();
    const { error } = await supabase.rpc("rpc_crear_auth_registro", {
        p_token: token,
        p_password: password,
    });
    if (error)
